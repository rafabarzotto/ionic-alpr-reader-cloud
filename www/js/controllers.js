...
.controller('DashCtrl', function($scope, $cordovaCamera, $timeout, $cordovaFileTransfer, $ionicPopup, $http, $ionicLoading) {
  ...
  $scope.teste = function() {
    $http({
      method: 'GET',
      url: 'http://localhost:3000/check_plate'
    }).then(function successCallback(response) {
      if (response.data.results.length == 0) {
        $scope.showAlert('Alerta', 'Não conseguiu reconhecer!');
      } else {
        $ionicPopup.alert({
          title: 'Resposta!',
  ...
  $scope.uploadFiles = function() {
    var server = "http://ec2-54-94-139-45.sa-east-1.compute.amazonaws.com:3000/upload";
